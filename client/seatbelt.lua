...
RegisterNetEvent('KGCore:Client:EnteredVehicle', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('seatbelt:client:UseHarness', function(ItemData) -- On Item Use (registered server side)
    ...
    if inVeh and class ~= 8 and class ~= 13 and class ~= 14 then
        if not harnessOn then
            ...
            KGCore.Functions.Progressbar("harness_equip", Lang:t('seatbelt.use_harness_progress'), 5000, false, true, {
            ...
            LocalPlayer.state:set("inv_busy", true, true)
            KGCore.Functions.Progressbar("harness_equip", Lang:t('seatbelt.remove_harness_progress'), 5000, false, true, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
        ...
        KGCore.Functions.Notify(Lang:t('seatbelt.no_car'), 'error')
    end
end)
