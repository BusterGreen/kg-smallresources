...
RegisterNetEvent('consumables:client:Eat', function(itemName)
    KGCore.Functions.Progressbar('eat_something', Lang:t('consumables.eat_progress'), 5000, false, true, {
        ...
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items[itemName], 'remove')
        TriggerServerEvent('consumables:server:addHunger', KGCore.Functions.GetPlayerData().metadata.hunger + Config.Consumables.eat[itemName])
        TriggerServerEvent('hud:server:RelieveStress', math.random(2, 4))
    end)
end)

RegisterNetEvent('consumables:client:Drink', function(itemName)
    KGCore.Functions.Progressbar('drink_something', Lang:t('consumables.drink_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items[itemName], 'remove')
        TriggerServerEvent('consumables:server:addThirst', KGCore.Functions.GetPlayerData().metadata.thirst + Config.Consumables.drink[itemName])
    end)
end)

RegisterNetEvent('consumables:client:DrinkAlcohol', function(itemName)
    KGCore.Functions.Progressbar('drink_alcohol', Lang:t('consumables.liqour_progress'), math.random(3000, 6000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        TriggerServerEvent('consumables:server:addThirst', KGCore.Functions.GetPlayerData().metadata.thirst + Config.Consumables.alcohol[itemName])
        TriggerServerEvent('hud:server:RelieveStress', math.random(2, 4))
        alcoholCount += 1
        AlcoholLoop()
        ...
        KGCore.Functions.Notify(Lang:t('consumables.canceled'), 'error')
    end)
end)

RegisterNetEvent('consumables:client:Custom', function(itemName)
    KGCore.Functions.TriggerCallback('consumables:itemdata', function(data)
        KGCore.Functions.Progressbar('custom_consumable', data.progress.label, data.progress.time, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true
            ...
            if data.replenish.type then
                TriggerServerEvent('consumables:server:add' .. data.replenish.type, KGCore.Functions.GetPlayerData().metadata[string.lower(data.replenish.type)] + data.replenish.replenish)
            end
...
RegisterNetEvent('consumables:client:Cokebaggy', function()
    local ped = PlayerPedId()
    KGCore.Functions.Progressbar('snort_coke', Lang:t('consumables.coke_progress'), math.random(5000, 8000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['cokebaggy'], 'remove')
...
RegisterNetEvent('consumables:client:Crackbaggy', function()
    local ped = PlayerPedId()
    KGCore.Functions.Progressbar('snort_coke', Lang:t('consumables.crack_progress'), math.random(7000, 10000), false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['crack_baggy'], 'remove')
...
RegisterNetEvent('consumables:client:EcstasyBaggy', function()
    KGCore.Functions.Progressbar('use_ecstasy', Lang:t('consumables.ecstasy_progress'), 3000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['xtcbaggy'], 'remove')
        ecstasyEffect()
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), 'mp_suicide', 'pill', 1.0)
        KGCore.Functions.Notify(Lang:t('consumables.canceled'), 'error')
...
RegisterNetEvent('consumables:client:oxy', function()
    KGCore.Functions.Progressbar('use_oxy', Lang:t('consumables.healing_progress'), 2000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['oxy'], 'remove')
...
RegisterNetEvent('consumables:client:meth', function()
    KGCore.Functions.Progressbar('snort_meth', Lang:t('consumables.meth_progress'), 1500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['meth'], 'remove')
        TriggerEvent('evidence:client:SetStatus', 'widepupils', 300)
...
RegisterNetEvent('consumables:client:UseJoint', function()
    KGCore.Functions.Progressbar('smoke_joint', Lang:t('consumables.joint_progress'), 1500, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
    }, {}, {}, {}, function() -- Done
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['joint'], 'remove')
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            KGCore.Functions.PlayAnim('timetable@gardener@smoking_joint', 'smoke_idle', false)
        else
            KGCore.Functions.PlayAnim('timetable@gardener@smoking_joint', 'smoke_idle', false)
        end
...
RegisterNetEvent('consumables:client:UseParachute', function()
    equipParachuteAnim()
    KGCore.Functions.Progressbar('use_parachute', Lang:t('consumables.use_parachute_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
        ...
        local ped = PlayerPedId()
        TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['parachute'], 'remove')
        GiveWeaponToPed(ped, `GADGET_PARACHUTE`, 1, false, false)
        local parachuteData = {
            outfitData = { ['bag'] = { item = 7, texture = 0 } } -- Adding Parachute Clothing
        }
        TriggerEvent('kg-clothing:client:loadOutfit', parachuteData)
...
RegisterNetEvent('consumables:client:ResetParachute', function()
    if parachuteEquipped then
        equipParachuteAnim()
        KGCore.Functions.Progressbar('reset_parachute', Lang:t('consumables.pack_parachute_progress'), 40000, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            local ped = PlayerPedId()
            TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['parachute'], 'add')
            local parachuteResetData = {
                outfitData = { ['bag'] = { item = 0, texture = 0 } } -- Removing Parachute Clothing
            }
            TriggerEvent('kg-clothing:client:loadOutfit', parachuteResetData)
            TaskPlayAnim(ped, 'clothingshirt', 'exit', 8.0, 1.0, -1, 49, 0, false, false, false)
            TriggerServerEvent('consumables:server:AddParachute')
            parachuteEquipped = false
        end)
    else
        KGCore.Functions.Notify(Lang:t('consumables.no_parachute'), 'error')
    end
end)

RegisterNetEvent('consumables:client:UseArmor', function()
    if GetPedArmour(PlayerPedId()) >= 75 then
        KGCore.Functions.Notify(Lang:t('consumables.armor_full'), 'error')
        return
    end
    KGCore.Functions.Progressbar('use_armor', Lang:t('consumables.armor_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('consumables:client:UseHeavyArmor', function()
    ...
    KGCore.Functions.Progressbar('use_heavyarmor', Lang:t('consumables.heavy_armor_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
...
RegisterNetEvent('consumables:client:ResetArmor', function()
    ...
    if currVest ~= nil and currVestTexture ~= nil then
        KGCore.Functions.Progressbar('remove_armor', Lang:t('consumables.remove_armor_progress'), 2500, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true,
            ...
            SetPedArmour(ped, 0)
            TriggerEvent('kg-inventory:client:ItemBox', KGCore.Shared.Items['heavyarmor'], 'add')
            TriggerServerEvent('consumables:server:resetArmor')
        end)
    else
        KGCore.Functions.Notify(Lang:t('consumables.armor_empty'), 'error')
    end
end)
...
--         TriggerEvent("kg-inventory:client:ItemBox", KGCore.Shared.Items["smoketrailred"], "remove")
--     else
--         KGCore.Functions.Notify("You need to have a paracute to activate smoke!", "error")
--     end
-- end)
