            ...
            if driver == ped and not washingVeh then
                if IsControlPressed(0, 38) then
                    if dirtLevel > Config.CarWash.dirtLevel then
                        TriggerServerEvent('kg-carwash:server:washCar')
                    else
                        KGCore.Functions.Notify(Lang:t('wash.dirty'), 'error')
                    end
                    listen = false
                    break
                end
...
RegisterNetEvent('kg-carwash:client:washCar', function()
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsIn(ped, false)
    washingVeh = true
    KGCore.Functions.Progressbar('search_cabin', Lang:t('wash.in_progress'), math.random(4000, 8000), false, true, {
        ...
        KGCore.Functions.Notify(Lang:t('wash.cancel'), 'error')
        washingVeh = false
    end)
end)
...
CreateThread(function()
    for k, v in pairs(Config.CarWash.locations) do
        if Config.UseTarget then
            exports["kg-target"]:AddBoxZone('carwash_'..k, v.coords, v.length, v.width, {
                name = 'carwash_'..k,
                debugPoly = false,
                heading = v.heading,
                minZ = v.coords.z - 5,
            ...
            washCombo:onPlayerInOut(function(isPointInside)
                if isPointInside and IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['kg-core']:DrawText(Lang:t('wash.wash_vehicle'),'left')
                    if not listen then
                        listen = true
                        washLoop()
                    end
