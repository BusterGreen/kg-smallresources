...
RegisterCommand('tackle', function()
    local closestPlayer, distance = KGCore.Functions.GetClosestPlayer()
    local ped = PlayerPedId()
    if distance ~= -1 and distance < 2 and GetEntitySpeed(ped) > 2.5 and not IsPedInAnyVehicle(ped, false) and not KGCore.Functions.GetPlayerData().metadata.ishandcuffed and not IsPedRagdoll(ped) then
