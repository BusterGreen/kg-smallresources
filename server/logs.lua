...
RegisterNetEvent('kg-log:server:CreateLog', function(name, title, color, message, tagEveryone, imageUrl)
    local postData = {}
    ...
    if Config.Logging == 'discord' then
        ...
        local embedData = {
            {
                ...
                ['author'] = {
                    ['name'] = 'KGCore Logs',
                    ['icon_url'] = 'https://raw.githubusercontent.com/GhzGarage/kg-media-kit/main/Display%20Pictures/Logo%20-%20Display%20Picture%20-%20Stylized%20-%20Red.png',
                },
        ...
        if #logQueue[name] >= 10 then
            if tag then
                postData = { username = 'KG Logs', content = '@everyone', embeds = {} }
            else
                postData = { username = 'KG Logs', embeds = {} }
            end
    ...
    while true do
        ...
        if timer >= 60 then -- If 60 seconds have passed, post the logs
            ...
            for name, queue in pairs(logQueue) do
                if #queue > 0 then
                    local postData = { username = 'KG Logs', embeds = {} }
                    for i = 1, #queue do
...
KGCore.Commands.Add('testwebhook', 'Test Your Discord Webhook For Logs (God Only)', {}, false, function()
    TriggerEvent('kg-log:server:CreateLog', 'testwebhook', 'Test Webhook', 'default', 'Webhook setup successfully')
end, 'god')
